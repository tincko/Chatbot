...
def split_think_and_answer(text: str):
    ...
    if start != -1 and end != -1 and end > start:
        ...
        # Si por algún motivo no viene bien cerrado, tratamos todo como respuesta
        think = ""
        answer = text.strip()
...
def postprocess_psico_reply(text: str) -> str:
    # 1) Si el modelo larga algo tipo "<think>... Posible respuesta: ..."
    #    nos quedamos solo con lo que viene después de "Posible respuesta:"
    markers = ["Posible respuesta:", "Respuesta al paciente:", "Mensaje para el paciente:"]
    for m in markers:
        if m in text:
            text = text.split(m, 1)[1].strip()
            break

    # 2) Si aún hay <think> o "Análisis" arriba, los eliminamos
    if "<think>" in text:
        text = text.replace("<think>", "")
    # Podés limpiar más patrones si hace falta
...
for profile in patient_profiles:
    ...
    for turn in range(10):
        ...
        reply_psico_raw = ask_model(MODEL_PSICO, history_psico)

        # 1) separar razonamiento interno y respuesta visible
        think_psico, reply_psico = split_think_and_answer(reply_psico_raw)

        # 2) loguear el <think> en un log separado
        if think_psico:
            log("===== PSICO THINK =====\n" + think_psico + "\n\n")

        # 3) loguear también la respuesta visible (si querés)
        log("===== PSICO RESPUESTA =====\n" + reply_psico + "\n\n")

        # 4) SOLO la respuesta visible entra en el historial de la conversación
